...
const registerProveedoresRoutes = (app, { pool, verifyToken }) => {
    ...
    app.post('/proveedores', verifyToken, async (req, res, _next) => {
        ...
        try {
            ...
            if (exist.rows.length > 0) {
                if (exist.rows[0].activo)
                    ...
                    await pool.query('UPDATE proveedores SET nombre=$1, email=$2, telefono1=$3, telefono2=$4, direccion=$5, estado_venezuela=$6, rubro=$7, activo=true WHERE id=$8', [nombre, emailFmt, telefono1, telefono2, direccion, estado_venezuela, rubro || null, exist.rows[0].id]);
                    return res.json({ status: 'success', message: 'El proveedor estaba oculto, ha sido reactivado y actualizado.' });
                }
            }
            await pool.query('INSERT INTO proveedores (condominio_id, identificador, nombre, email, telefono1, telefono2, direccion, estado_venezuela, rubro) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [condoId, identificador, nombre, emailFmt, telefono1, telefono2, direccion, estado_venezuela, rubro || null]);
    ...
    app.post('/proveedores/lote', verifyToken, async (req, res, _next) => {
        const user = asAuthUser(req.user);
        ...
        try {
            ...
            for (const item of proveedores) {
                ...
                if (!isValidEmail(emailFmt))
                    throw new Error(`Correo invÃ¡lido para el proveedor con RIF ${rifFmt || '(sin RIF)'}.`);
                ...
                if (exist.rows.length > 0) {
                    if (exist.rows[0].activo) {
                        ...
                        await pool.query('UPDATE proveedores SET nombre=$1, email=$2, telefono1=$3, telefono2=$4, direccion=$5, estado_venezuela=$6, rubro=$7, activo=true WHERE id=$8', [item.nombre, emailFmt, item.telefono1, item.telefono2 || null, item.direccion, item.estado_venezuela, item.rubro || null, exist.rows[0].id]);
                    }
                }
                else {
                    // Si no existe, lo insertamos
                    await pool.query('INSERT INTO proveedores (condominio_id, identificador, nombre, email, telefono1, telefono2, direccion, estado_venezuela, rubro) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [condoId, rifFmt, item.nombre, emailFmt, item.telefono1, item.telefono2 || null, item.direccion, item.estado_venezuela, item.rubro || null]);
                }
            }
    ...
    app.put('/proveedores/:id', verifyToken, async (req, res, _next) => {
        ...
        if (!isValidEmail(emailFmt))
            return res.status(400).json({ error: 'Correo electrÃ³nico invÃ¡lido.' });
        try {
            await pool.query('UPDATE proveedores SET nombre=$1, email=$2, telefono1=$3, telefono2=$4, direccion=$5, estado_venezuela=$6, rubro=$7 WHERE id=$8', [nombre, emailFmt, telefono1, telefono2, direccion, estado_venezuela, rubro || null, proveedorId]);
    ...
    app.delete('/proveedores/:id', verifyToken, async (req, res, _next) => {
        try {
            const proveedorId = asString(req.params.id);
    ...
    app.get('/proveedores', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const c = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
