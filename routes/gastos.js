...
const toNumber = (value) => parseFloat(String(value ?? 0)) || 0;
const fetchBcvRateToday = async () => {
...
const registerGastosRoutes = (app, { pool, verifyToken, parseLocaleNumber, addMonths, formatMonthText }) => {
    ...
    app.post('/gastos', verifyToken, upload.fields([{ name: 'factura_img', maxCount: 1 }, { name: 'soportes', maxCount: 4 }]), async (req, res, _next) => {
        const user = asAuthUser(req.user);
        if (!user.cedula.startsWith('J'))
    ...
    app.get('/gastos', verifyToken, async (req, res, _next) => {
        const user = asAuthUser(req.user);
        if (!user.cedula.startsWith('J'))
    ...
    app.delete('/gastos/:id', verifyToken, async (req, res, _next) => {
        try {
            ...
            if (cuotasCheck.rows.length > 0)
                return res.status(400).json({ status: 'error', message: 'No puedes eliminar un gasto con cuotas procesadas.' });
    ...
    app.get('/preliminar', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const condoRes = await pool.query('SELECT id, mes_actual, metodo_division FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
    ...
    app.put('/metodo-division', verifyToken, async (req, res, _next) => {
        const user = asAuthUser(req.user);
        ...
        try {
            await pool.query('UPDATE condominios SET metodo_division = $1 WHERE admin_user_id = $2', [metodo, user.id]);
    ...
    app.post('/cerrar-ciclo', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            await pool.query('BEGIN');
            const condoRes = await pool.query(`SELECT
            ...
            const propRes = await pool.query('SELECT id, identificador, alicuota, saldo_actual FROM propiedades WHERE condominio_id = $1', [condo_id]);
            const cuotasRes = await pool.query(`SELECT
                    g.id AS gasto_id,
                    g.concepto,
                 ...
                 WHERE g.condominio_id = $1
                   ...
                   AND gc.estado = 'Pendiente'`, [condo_id, mes_actual]);
            const fondosRes = await pool.query(`SELECT
                    f.id,
            ...
            for (const p of propRes.rows) {
                ...
                const participantesRes = await pool.query(`SELECT up.rol, u.nombre
                     FROM usuarios_propiedades up
                     JOIN users u ON u.id = up.user_id
                     WHERE up.propiedad_id = $1
                       AND up.rol IN ('Propietario', 'Inquilino')
                     ORDER BY CASE WHEN up.rol = 'Propietario' THEN 0 ELSE 1 END, up.id ASC`, [p.id]);
                ...
                for (const c of cuotasRes.rows) {
                    ...
                    if (c.tipo === 'Comun' || c.tipo === 'Extra') {
                        if (metodo_division === 'Partes Iguales')
                            cuotaPropiedadUsd = parseFloat(String(c.monto_cuota_usd)) / propRes.rows.length;
                        else
                            cuotaPropiedadUsd = parseFloat(String(c.monto_cuota_usd)) * (parseFloat(String(p.alicuota)) / 100);
                    }
                        ...
                        if (metodo_division === 'Partes Iguales')
                            cuotaPropiedadUsd = parseFloat(String(c.monto_cuota_usd)) / parseInt(propsZona.rows[0].count, 10);
                        else {
                ...
                if (total_deuda > 0) {
                    ...
                    const recRes = await pool.query("INSERT INTO recibos (propiedad_id, mes_cobro, monto_usd, estado) VALUES ($1, $2, $3, 'Aviso de Cobro') RETURNING id", [p.id, formatMonthText(mes_actual), deudaFinal]);
                    ...
                    await pool.query('UPDATE recibos SET snapshot_jsonb = $1, snapshot_version = 1 WHERE id = $2', [JSON.stringify(snapshotAviso), nuevoReciboId]);
                    // 2. Aumentar la deuda global
                    await pool.query('UPDATE propiedades SET saldo_actual = saldo_actual + $1 WHERE id = $2', [deudaFinal, p.id]);
                    ...
                    if (viejoSaldo < 0) {
                        ...
                        if (saldoAFavor >= total_deuda) {
                            ...
                            await pool.query(`UPDATE recibos
    ...
    app.get('/gastos-extras-procesados', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
