...
const buildSeedGastoNota = (concepto) => {
...
const registerDashboardRoutes = (app, { pool, verifyToken }) => {
    const seederEnabled = ['1', 'true', 'yes', 'on'].includes(String(process.env.ENABLE_TEST_SEEDER || '').trim().toLowerCase());
    const buildCedula = (_faker, seedNum) => `V${String(10000000 + seedNum).slice(-8)}`;
    ...
    app.get('/mis-propiedades', verifyToken, async (req, res, _next) => {
        try {
    ...
    app.get('/mis-finanzas', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const queryDeuda = `
            ...
            `;
            const resultDeuda = await pool.query(queryDeuda, [user.id]);
            res.json({
                status: 'success',
                finanzas: {
    ...
    const adminResumenHandler = async (req, res) => {
        try {
            const user = asAuthUser(req.user);
            ...
            const [liquidezRes, porCobrarRes, cuentasPorPagarRes, egresosMesRes] = await Promise.all([
                pool.query(`SELECT COALESCE(SUM(f.saldo_actual), 0)::text AS total
                     FROM fondos f
                     WHERE f.condominio_id = $1`, [condominioId]),
                pool.query(`SELECT COALESCE(SUM(GREATEST(r.monto_usd - COALESCE(r.monto_pagado_usd, 0), 0)), 0)::text AS total
                     FROM recibos r
                     INNER JOIN propiedades p ON p.id = r.propiedad_id
                     WHERE p.condominio_id = $1
                       AND r.estado IN ('Aviso de Cobro', 'Pendiente')`, [condominioId]),
                pool.query(`SELECT COALESCE(SUM(gc.monto_cuota_usd), 0)::text AS total
                     FROM gastos_cuotas gc
                     INNER JOIN gastos g ON g.id = gc.gasto_id
                     WHERE g.condominio_id = $1
                       AND gc.estado = 'Pendiente'`, [condominioId]),
                pool.query(`SELECT COALESCE(SUM(pp.monto_usd), 0)::text AS total
                     FROM pagos_proveedores pp
                     JOIN gastos g ON pp.gasto_id = g.id
                     WHERE g.condominio_id = $1
                       AND EXTRACT(MONTH FROM pp.fecha_pago) = EXTRACT(MONTH FROM CURRENT_DATE)
                       AND EXTRACT(YEAR FROM pp.fecha_pago) = EXTRACT(YEAR FROM CURRENT_DATE)`, [condominioId]),
            ]);
    ...
    app.get('/admin-resumen', verifyToken, (req, res) => {
        void adminResumenHandler(req, res);
    });
    app.get('/api/dashboard/admin-resumen', verifyToken, (req, res) => {
        void adminResumenHandler(req, res);
    });
    app.get('/admin-graficos', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const condoRes = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
            if (condoRes.rows.length === 0) {
            ...
            const [ingresosRes, egresosRes, gastosRubrosRes] = await Promise.all([
                pool.query(`SELECT to_char(date_trunc('month', p.fecha_pago::timestamp), 'YYYY-MM') AS mes,
                     ...
                     GROUP BY 1
                     ORDER BY 1 ASC`, [condominioId]),
                pool.query(`SELECT to_char(date_trunc('month', pp.fecha_pago::timestamp), 'YYYY-MM') AS mes,
                     ...
                     GROUP BY 1
                     ORDER BY 1 ASC`, [condominioId]),
                pool.query(`SELECT COALESCE(NULLIF(BTRIM(pr.rubro), ''), 'Otros') AS name,
                     ...
                     GROUP BY 1
                     ORDER BY SUM(g.monto_usd) DESC`, [condominioId]),
            ]);
            const toNumber = (value) => {
                const parsed = parseFloat(String(value ?? '0'));
                return Number.isFinite(parsed) ? parsed : 0;
            ...
            const ingresosMap = new Map(ingresosRes.rows.map((row) => [row.mes, toNumber(row.total)]));
            const egresosMap = new Map(egresosRes.rows.map((row) => [row.mes, toNumber(row.total)]));
            const monthKeys = Array.from({ length: 6 }, (_v, idx) => {
                const d = new Date();
                d.setDate(1);
    ...
    app.get('/admin-movimientos', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const condoRes = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
            if (condoRes.rows.length === 0) {
            ...
            const [ultimosPagosRes, pagosPendientesRes, gastosPendientesRes] = await Promise.all([
                pool.query(`SELECT
                     ...
                     LIMIT 5`, [condominioId]),
                pool.query(`SELECT COUNT(*)::int AS total
                     FROM pagos p
                     INNER JOIN propiedades pr ON pr.id = p.propiedad_id
                     WHERE pr.condominio_id = $1
                       AND p.estado = 'Pendiente'`, [condominioId]),
                pool.query(`SELECT COUNT(*)::int AS total
                     FROM gastos_cuotas gc
                     INNER JOIN gastos g ON g.id = gc.gasto_id
                     WHERE g.condominio_id = $1
    ...
    app.get('/cuentas-por-cobrar', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const c = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
            if (c.rows.length === 0)
    ...
    app.post('/dashboard-admin/seed-prueba', verifyToken, async (req, res, _next) => {
        try {
            if (!seederEnabled) {
            ...
            const condoRes = await pool.query('SELECT id, mes_actual FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
            if (condoRes.rows.length === 0) {
                throw new Error('Condominio no encontrado para este administrador.');
            }
            const condoId = condoRes.rows[0].id;
            const seedUsersRes = await pool.query(`SELECT DISTINCT u.id
                 FROM users u
                 JOIN usuarios_propiedades up ON up.user_id = u.id
                 JOIN propiedades p ON p.id = up.propiedad_id
                 WHERE p.condominio_id = $1
                   AND u.email ILIKE '%@seed.habioo.test'`, [condoId]);
            ...
            await pool.query(`DELETE FROM pagos p
                 ...
                 WHERE p.recibo_id = r.id
                   ...
                   AND pr.condominio_id = $1`, [condoId]);
            await pool.query(`DELETE FROM pagos p
            ...
            await pool.query(`DELETE FROM recibos r
                 ...
                 WHERE r.propiedad_id = pr.id
                   AND pr.condominio_id = $1`, [condoId]);
            await pool.query(`DELETE FROM transferencias WHERE condominio_id = $1`, [condoId]);
            await pool.query(`DELETE FROM pagos_proveedores
                 WHERE fondo_id IN (SELECT id FROM fondos WHERE condominio_id = $1)
                    OR gasto_id IN (SELECT id FROM gastos WHERE condominio_id = $1)`, [condoId]);
            await pool.query(`DELETE FROM movimientos_fondos
                 WHERE fondo_id IN (SELECT id FROM fondos WHERE condominio_id = $1)`, [condoId]);
            await pool.query(`DELETE FROM historial_saldos_inmuebles
                 WHERE propiedad_id IN (SELECT id FROM propiedades WHERE condominio_id = $1)`, [condoId]);
            await pool.query(`DELETE FROM usuarios_propiedades
                 WHERE propiedad_id IN (SELECT id FROM propiedades WHERE condominio_id = $1)`, [condoId]);
            await pool.query(`DELETE FROM propiedades_zonas
                 WHERE propiedad_id IN (SELECT id FROM propiedades WHERE condominio_id = $1)`, [condoId]);
            await pool.query("DELETE FROM gastos_cuotas WHERE gasto_id IN (SELECT id FROM gastos WHERE condominio_id = $1)", [condoId]);
            ...
            if (seedUserIdsToDelete.length > 0) {
                await pool.query(`DELETE FROM users
                     WHERE id = ANY($1::int[])
                       AND id <> $2`, [seedUserIdsToDelete, user.id]);
            }
            ...
            const cuentaPrincipalBs = await pool.query(`INSERT INTO cuentas_bancarias
            ...
            const cuentaPagoMovilBs = await pool.query(`INSERT INTO cuentas_bancarias
                    (condominio_id, numero_cuenta, nombre_banco, apodo, tipo, nombre_titular, cedula_rif, telefono, es_predeterminada, acepta_transferencia, acepta_pago_movil, pago_movil_telefono, pago_movil_cedula_rif)
                 VALUES
            ...
            const cuentaSecundariaBs = await pool.query(`INSERT INTO cuentas_bancarias
                    (condominio_id, numero_cuenta, nombre_banco, apodo, tipo, nombre_titular, cedula_rif, telefono, es_predeterminada, acepta_transferencia, acepta_pago_movil, pago_movil_telefono, pago_movil_cedula_rif)
                 VALUES
            ...
            const cuentaUsd = await pool.query(`INSERT INTO cuentas_bancarias
                    (condominio_id, numero_cuenta, nombre_banco, apodo, tipo, nombre_titular, cedula_rif, telefono, es_predeterminada, acepta_transferencia, acepta_pago_movil, pago_movil_telefono, pago_movil_cedula_rif)
                 VALUES
            ...
            await pool.query(`INSERT INTO fondos
            ...
            const propConfigs = Array.from({ length: 20 }, (_, idx) => {
                ...
                let saldo = 0;
                if (i % 3 === 1)
                    saldo = Number((10 + i * 1.75).toFixed(2));
                if (i % 3 === 2)
                    saldo = Number((-6 - i * 1.35).toFixed(2));
                return {
            ...
            for (const pc of propConfigs) {
                const p = await pool.query(`INSERT INTO propiedades (condominio_id, identificador, alicuota, zona_id, saldo_actual)
                     VALUES ($1, $2, 5.00, $3, $4)
                     RETURNING id`, [condoId, pc.iden, pc.zona, pc.saldo]);
                props.push(p.rows[0].id);
                await pool.query('INSERT INTO propiedades_zonas (propiedad_id, zona_id) VALUES ($1, $2)', [p.rows[0].id, pc.zona]);
                if (pc.saldo !== 0) {
                    await pool.query(`INSERT INTO historial_saldos_inmuebles (propiedad_id, tipo, monto, nota, fecha)
                ...
                const ownerUser = await pool.query(`INSERT INTO users (cedula, nombre, email, telefono, password)
                     VALUES ($1, $2, $3, $4, $5)
                     RETURNING id`, [propCedula, propNombre, propEmail, propTelefono, propCedula]);
                await pool.query(`INSERT INTO usuarios_propiedades (user_id, propiedad_id, rol)
                     VALUES ($1, $2, 'Propietario')`, [ownerUser.rows[0].id, p.rows[0].id]);
            }
            const proveedorIds = [];
            for (let i = 1; i <= 8; i += 1) {
                const seedNum = Date.now() + i;
                const prov = await pool.query(`INSERT INTO proveedores
            ...
            for (const g of gastosConfig) {
                ...
                const gas = await pool.query(`INSERT INTO gastos
                     ...
                     RETURNING id`, [condoId, proveedorId, g.concepto, g.usd * 40, g.usd, totalCuotas, notaSeed, clasificacionSeed, g.tipo, g.zona, g.prop, fechaGastoSeed]);
                ...
                for (let i = 1; i <= totalCuotas; i += 1) {
                    ...
                    await pool.query(`INSERT INTO gastos_cuotas (gasto_id, numero_cuota, monto_cuota_usd, mes_asignado, estado)
                         VALUES ($1, $2, $3, $4, 'Pendiente')`, [gas.rows[0].id, i, cuota, addMonthsSeed(mesActual, i - 1)]);
                }
            }
