...
const toTitleCase = (value) => normalizeWhitespace(value)
    ...
    .join('-'))
    .join(' ');
const normalizeIdentifier = (value) => normalizeWhitespace(value).toUpperCase();
const normalizeDoc = (value) => normalizeWhitespace(value).toUpperCase();
const registerPropiedadesRoutes = (app, { pool, verifyToken }) => {
    ...
    const userHasLinksOutsideProperty = async (userId, propiedadId) => {
        const r = await pool.query(`SELECT 1 AS ok
             FROM usuarios_propiedades
             WHERE user_id = $1
               AND propiedad_id <> $2
             LIMIT 1`, [userId, propiedadId]);
        return r.rows.length > 0;
    };
    const updateUserIfExclusiveToProperty = async (userId, propiedadId, values, failIfShared = false) => {
        ...
        const currentRes = await pool.query('SELECT id, cedula, nombre, email, telefono FROM users WHERE id = $1 LIMIT 1', [userId]);
        ...
        await pool.query('UPDATE users SET cedula = $1, nombre = $2, email = $3, telefono = $4 WHERE id = $5', [nextCedula, nextNombre, nextEmail, nextTelefono, userId]);
        if (values.password) {
    ...
    app.get('/propiedades-admin', verifyToken, async (req, res, _next) => {
        try {
            ...
            `, [condominioId]);
            const gastosRes = await pool.query('SELECT COUNT(*)::text AS count FROM gastos WHERE condominio_id = $1', [condominioId]);
    ...
    app.get('/propiedades-admin/propietarios-existentes', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const condominioId = await getCondominioIdByAdmin(user.id);
            if (!condominioId) {
    ...
    app.get('/propiedades-admin/:id/copropietarios', verifyToken, async (req, res, _next) => {
        ...
        try {
            ...
            const propRes = await pool.query('SELECT id FROM propiedades WHERE id = $1 AND condominio_id = $2 LIMIT 1', [propiedadId, condominioId]);
            ...
            const result = await pool.query(`SELECT
                 ...
                 ORDER BY up.id ASC`, [propiedadId]);
    ...
    app.post('/propiedades-admin/:id/copropietarios', verifyToken, async (req, res, _next) => {
        const propiedadIdRaw = asString(req.params.id);
        const propiedadId = parseInt(propiedadIdRaw, 10);
        if (!Number.isFinite(propiedadId) || propiedadId <= 0) {
            return res.status(400).json({ error: 'ID de inmueble inválido.' });
        ...
        try {
            ...
            const ownerRes = await pool.query(`SELECT u.cedula
                 FROM usuarios_propiedades up
                 INNER JOIN users u ON u.id = up.user_id
                 WHERE up.propiedad_id = $1
            ...
            let userId;
            const userByCedula = await pool.query('SELECT id FROM users WHERE cedula = $1 LIMIT 1', [cedulaNormalized]);
            if (userByCedula.rows.length > 0) {
                userId = userByCedula.rows[0].id;
                await updateUserIfExclusiveToProperty(userId, propiedadId, { nombre: nombreNormalized, email: correo, telefono }, false);
            }
            else {
                if (correo) {
                    const userByEmail = await pool.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1', [correo]);
                ...
                const insertUser = await pool.query('INSERT INTO users (cedula, nombre, email, telefono, password) VALUES ($1, $2, $3, $4, $5) RETURNING id', [cedulaNormalized, nombreNormalized, correo, telefono, cedulaNormalized]);
                userId = insertUser.rows[0].id;
            }
            const existingAnyRole = await pool.query('SELECT id, rol FROM usuarios_propiedades WHERE propiedad_id = $1 AND user_id = $2 LIMIT 1', [propiedadId, userId]);
            if (existingAnyRole.rows.length > 0) {
                const currentRole = String(existingAnyRole.rows[0].rol || '');
                if (currentRole === 'Copropietario') {
                    await pool.query('UPDATE usuarios_propiedades SET acceso_portal = $1 WHERE id = $2', [accesoPortal, existingAnyRole.rows[0].id]);
            ...
            await pool.query('INSERT INTO usuarios_propiedades (user_id, propiedad_id, rol, acceso_portal) VALUES ($1, $2, $3, $4)', [userId, propiedadId, 'Copropietario', accesoPortal]);
            await pool.query('COMMIT');
    ...
    app.put('/propiedades-admin/:id/copropietarios/:linkId', verifyToken, async (req, res, _next) => {
        const propiedadIdRaw = asString(req.params.id);
        ...
        try {
            ...
            const copropRes = await pool.query(`SELECT up.id, up.user_id
                 FROM usuarios_propiedades up
                 INNER JOIN propiedades p ON p.id = up.propiedad_id
                 WHERE up.id = $1
                 ...
                 LIMIT 1`, [linkId, propiedadId, condominioId]);
            ...
            const cedulaConflict = await pool.query('SELECT id FROM users WHERE cedula = $1 AND id <> $2 LIMIT 1', [cedulaNormalized, userId]);
            if (cedulaConflict.rows.length > 0) {
                await pool.query('ROLLBACK');
                return res.status(409).json({ error: 'La cédula ingresada ya pertenece a otro usuario.' });
            }
            if (correo) {
                const emailConflict = await pool.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1) AND id <> $2 LIMIT 1', [correo, userId]);
                if (emailConflict.rows.length > 0) {
                    await pool.query('ROLLBACK');
                    return res.status(409).json({ error: 'El correo ingresado ya pertenece a otro usuario.' });
                }
            }
            await updateUserIfExclusiveToProperty(userId, propiedadId, { cedula: cedulaNormalized, nombre: nombreNormalized, email: correo, telefono }, true);
            await pool.query('UPDATE usuarios_propiedades SET acceso_portal = $1 WHERE id = $2', [accesoPortal, linkId]);
            await pool.query('COMMIT');
            return res.json({ status: 'success', message: 'Copropietario actualizado correctamente.' });
        }
        catch (err) {
    ...
    app.delete('/propiedades-admin/:id/copropietarios/:linkId', verifyToken, async (req, res, _next) => {
        const propiedadIdRaw = asString(req.params.id);
        const linkIdRaw = asString(req.params.linkId);
        const propiedadId = parseInt(propiedadIdRaw, 10);
        const linkId = parseInt(linkIdRaw, 10);
        ...
        try {
            ...
            const result = await pool.query(`DELETE FROM usuarios_propiedades up
                 USING propiedades p
                 WHERE up.id = $1
                   AND up.propiedad_id = $2
                   AND up.rol = 'Copropietario'
                   ...
                   AND p.condominio_id = $3
                 RETURNING up.id`, [linkId, propiedadId, condominioId]);
    ...
    app.delete('/propiedades-admin/eliminar-todos', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const condominioId = await getCondominioIdByAdmin(user.id);
            if (!condominioId) {
            ...
            await pool.query('BEGIN');
            await pool.query(`DELETE FROM pagos p
                 USING recibos r, propiedades pr
                 WHERE p.recibo_id = r.id
                   AND r.propiedad_id = pr.id
                   AND pr.condominio_id = $1`, [condominioId]);
            await pool.query(`DELETE FROM pagos p
                 USING propiedades pr
                 WHERE p.propiedad_id = pr.id
                   AND pr.condominio_id = $1`, [condominioId]);
            await pool.query(`DELETE FROM recibos r
                 USING propiedades pr
                 WHERE r.propiedad_id = pr.id
                   AND pr.condominio_id = $1`, [condominioId]);
            await pool.query(`DELETE FROM historial_saldos_inmuebles
                 WHERE propiedad_id IN (SELECT id FROM propiedades WHERE condominio_id = $1)`, [condominioId]);
            await pool.query(`DELETE FROM usuarios_propiedades
                 WHERE propiedad_id IN (SELECT id FROM propiedades WHERE condominio_id = $1)`, [condominioId]);
            await pool.query(`DELETE FROM propiedades_zonas
                 WHERE propiedad_id IN (SELECT id FROM propiedades WHERE condominio_id = $1)`, [condominioId]);
            const deletedRes = await pool.query(`WITH deleted AS (
                    DELETE FROM propiedades
                    WHERE condominio_id = $1
                    RETURNING id
                 )
                 SELECT COUNT(*)::text AS count FROM deleted`, [condominioId]);
            await pool.query('COMMIT');
    ...
    // 2. ESTADO DE CUENTA
    app.get('/propiedades-admin/:id/estado-cuenta', verifyToken, async (req, res, _next) => {
        const propiedadId = asString(req.params.id);
        try {
            // 1. Cargos (Recibos / Deudas)
            const recibos = await pool.query(`SELECT
                    'RECIBO' as tipo,
                    id as ref_id,
                    CASE
            ...
            const pagos = await pool.query(`SELECT
                 ...
                 FROM pagos
                 WHERE propiedad_id = $1 AND estado = 'Validado'`, [propiedadId]);
            // 3. Ajustes Manuales (Saldos a favor / Deudas cargadas a mano)
            const ajustes = await pool.query(`SELECT
                    'AJUSTE' as tipo,
                    id as ref_id,
                    nota as concepto,
    ...
    app.post('/propiedades-admin/lote', verifyToken, async (req, res, _next) => {
        ...
        try {
            ...
            const alicuotasLote = propiedades.map((item) => {
            ...
            const allAlicuotasCero = alicuotasLote.every((alicuota) => alicuota === 0);
            const allAlicuotasMayorCero = alicuotasLote.every((alicuota) => alicuota > 0);
            ...
            const insertPropsRes = await pool.query(`
            ...
            const propiedadIdByIdentificador = new Map();
            ...
            for (const item of propiedades) {
                ...
                await pool.query('INSERT INTO historial_saldos_inmuebles (propiedad_id, tipo, monto, nota) VALUES ($1, $2, $3, $4)', [propiedadId, 'SALDO_INICIAL', Math.abs(saldoBase), `Carga masiva Excel (${tipoSaldo})`]);
            }
            ...
            for (const item of propiedades) {
                ...
                let userId;
                const userRes = await pool.query('SELECT id FROM users WHERE cedula = $1 LIMIT 1', [cedula]);
                if (userRes.rows.length === 0) {
                    const insertUser = await pool.query('INSERT INTO users (cedula, nombre, email, telefono, password) VALUES ($1, $2, $3, $4, $5) RETURNING id', [cedula, nombre, correo, telefonoFinal, cedula]);
                    userId = insertUser.rows[0].id;
                }
                else {
                    userId = userRes.rows[0].id;
                    await updateUserIfExclusiveToProperty(userId, propiedadId, { nombre, email: correo, telefono: telefonoFinal }, false);
                }
                const linkUpdate = await pool.query('UPDATE usuarios_propiedades SET user_id = $1 WHERE propiedad_id = $2 AND rol = $3', [userId, propiedadId, 'Propietario']);
            ...
            const saldoPorPropiedad = new Map();
            ...
            for (const [propiedadId, saldo] of saldoPorPropiedad.entries()) {
                ...
                if (saldo !== 0) {
                    ...
                    await pool.query('INSERT INTO historial_saldos_inmuebles (propiedad_id, tipo, monto, nota) VALUES ($1, $2, $3, $4)', [propiedadId, 'SALDO_INICIAL', Math.abs(saldo), `Importacion Estado_Cuenta (${tipoSaldo})`]);
                }
            }
    ...
    app.post('/propiedades-admin', verifyToken, async (req, res, _next) => {
        const { identificador, alicuota, zona_id, prop_nombre, prop_cedula, prop_email, prop_email_secundario, prop_telefono, prop_telefono_secundario, prop_password, tiene_inquilino, inq_nombre, inq_cedula, inq_email, inq_telefono, inq_password, inq_permitir_acceso, monto_saldo_inicial, tiene_deuda_inicial, deudas_iniciales, propietario_modo, propietario_existente_id } = req.body;
        ...
        if (!isValidEmail(ownerEmail))
            return res.status(400).json({ error: 'Email del propietario invÃ¡lido.' });
        if (!isValidEmail(tenantEmail))
            return res.status(400).json({ error: 'Email del inquilino invÃ¡lido.' });
        try {
            const user = asAuthUser(req.user);
            await pool.query('BEGIN');
            const condominioId = await getCondominioIdByAdmin(user.id);
            ...
            if (propietarioModo === 'EXISTENTE') {
                ...
                const userRes = await pool.query('SELECT id FROM users WHERE cedula = $1', [propCedulaNormalized]);
                if (userRes.rows.length > 0) {
                    await pool.query('ROLLBACK');
                ...
                if (ownerEmail) {
                    const userByEmailRes = await pool.query('SELECT id, cedula FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1', [ownerEmail]);
                ...
                if (!userId) {
                    const insertRes = await pool.query('INSERT INTO users (cedula, nombre, email, email_secundario, telefono, telefono_secundario, password) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id', [propCedulaNormalized, propNombreNormalized, ownerEmail, prop_email_secundario || null, prop_telefono || null, prop_telefono_secundario || null, prop_password || propCedulaNormalized]);
                    userId = insertRes.rows[0].id;
                }
            }
            const propRes = await pool.query('INSERT INTO propiedades (condominio_id, identificador, alicuota, zona_id, saldo_actual) VALUES ($1, $2, $3, $4, $5) RETURNING id', [condominioId, identificadorNormalized, alicuotaNum, zona_id || null, saldoBase]);
            ...
            if (usarDeudaInicial) {
                for (const item of deudaItemsNormalizados) {
                    ...
                    await pool.query('INSERT INTO historial_saldos_inmuebles (propiedad_id, tipo, monto, nota) VALUES ($1, $2, $3, $4)', [nuevaPropId, 'SALDO_INICIAL', item.montoNeto, nota]);
                }
            }
            else if (saldoBase !== 0) {
                const tipoSaldo = saldoBase > 0 ? 'DEUDA' : 'FAVOR';
            ...
            if (tiene_inquilino && inqCedulaNormalized && inqNombreNormalized) {
                ...
                let tenantRes = await pool.query('SELECT id FROM users WHERE cedula = $1', [inqCedulaNormalized]);
                if (tenantRes.rows.length === 0) {
                    tenantRes = await pool.query('INSERT INTO users (cedula, nombre, email, telefono, password) VALUES ($1, $2, $3, $4, $5) RETURNING id', [inqCedulaNormalized, inqNombreNormalized, tenantEmail, inq_telefono || null, inq_password || inqCedulaNormalized]);
                }
                else {
                    const existingTenantId = tenantRes.rows[0].id;
                    await updateUserIfExclusiveToProperty(existingTenantId, nuevaPropId, { nombre: inqNombreNormalized, email: tenantEmail, telefono: inq_telefono || null, password: inq_password || null }, false);
                }
    ...
    app.put('/propiedades-admin/:id', verifyToken, async (req, res, _next) => {
        const propiedadId = asString(req.params.id);
        ...
        const alicuotaNum = parseFloat((alicuota || '0').toString().replace(',', '.')) || 0;
        if (!isValidEmail(ownerEmail))
            return res.status(400).json({ error: 'Email del propietario inválido.' });
        if (!isValidEmail(tenantEmail))
            return res.status(400).json({ error: 'Email del inquilino inválido.' });
        try {
            await pool.query('BEGIN');
            ...
            if (propCedulaNormalized && propNombreNormalized) {
                ...
                const currentOwnerLinkRes = await pool.query('SELECT id, user_id FROM usuarios_propiedades WHERE propiedad_id = $1 AND rol = $2 LIMIT 1', [propiedadId, 'Propietario']);
                const currentOwnerUserId = currentOwnerLinkRes.rows[0]?.user_id || null;
                let userRes = await pool.query('SELECT id FROM users WHERE cedula = $1', [propCedulaNormalized]);
                let userId = null;
                if (userRes.rows.length === 0) {
                    ...
                    if (currentOwnerUserId) {
                        const updatedCurrentOwner = await updateUserIfExclusiveToProperty(currentOwnerUserId, propiedadIdNum, { cedula: propCedulaNormalized, nombre: propNombreNormalized, email: ownerEmail, telefono: prop_telefono || null, password: prop_password || null }, false);
                        if (updatedCurrentOwner) {
                            await pool.query('UPDATE users SET email_secundario = $1, telefono_secundario = $2 WHERE id = $3', [ownerEmailSec, ownerTelefonoSec, currentOwnerUserId]);
                    ...
                    if (!userId) {
                        userRes = await pool.query('INSERT INTO users (cedula, nombre, email, email_secundario, telefono, telefono_secundario, password) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id', [propCedulaNormalized, propNombreNormalized, ownerEmail, ownerEmailSec, prop_telefono || null, ownerTelefonoSec, prop_password || propCedulaNormalized]);
                        userId = userRes.rows[0].id;
                    }
                }
                else {
                    const existingOwnerId = userRes.rows[0].id;
                    await updateUserIfExclusiveToProperty(existingOwnerId, propiedadIdNum, { nombre: propNombreNormalized, email: ownerEmail, telefono: prop_telefono || null, password: prop_password || null }, false);
                    // Actualizar campos secundarios adicionales
                    await pool.query('UPDATE users SET email_secundario = $1, telefono_secundario = $2 WHERE id = $3', [ownerEmailSec, ownerTelefonoSec, existingOwnerId]);
                    userId = existingOwnerId;
                }
                const linkRes = await pool.query('SELECT id, user_id FROM usuarios_propiedades WHERE propiedad_id = $1 AND rol = $2', [propiedadId, 'Propietario']);
            ...
            if (tiene_inquilino && inqCedulaNormalized && inqNombreNormalized) {
                ...
                if (tenantRes.rows.length === 0) {
                    ...
                    const existingTenantId = tenantRes.rows[0].id;
                    const propiedadIdNum = parseInt(propiedadId, 10) || 0;
                    await updateUserIfExclusiveToProperty(existingTenantId, propiedadIdNum, { nombre: inqNombreNormalized, email: tenantEmail, telefono: inq_telefono || null, password: inq_password || null }, false);
                }
                const tenantId = tenantRes.rows[0].id;
                const tenantLink = await pool.query('SELECT id, user_id FROM usuarios_propiedades WHERE propiedad_id = $1 AND rol = $2', [propiedadId, 'Inquilino']);
                if (tenantLink.rows.length > 0) {
                    ...
                    await pool.query('INSERT INTO usuarios_propiedades (user_id, propiedad_id, rol, acceso_portal) VALUES ($1, $2, $3, $4)', [tenantId, propiedadId, 'Inquilino', inqPermitirAcceso]);
                }
            }
            else {
                const tenantLinks = await pool.query('SELECT user_id FROM usuarios_propiedades WHERE propiedad_id = $1 AND rol = $2', [propiedadId, 'Inquilino']);
    ...
    app.delete('/propiedades-admin/:id', verifyToken, async (req, res, _next) => {
        const propiedadIdRaw = asString(req.params.id);
        const propiedadId = parseInt(propiedadIdRaw, 10);
        if (!Number.isFinite(propiedadId) || propiedadId <= 0) {
            return res.status(400).json({ error: 'ID de inmueble inválido.' });
        ...
        try {
            ...
            const recibosRes = await pool.query('SELECT COUNT(*)::text AS count FROM recibos WHERE propiedad_id = $1', [propiedadId]);
            ...
            const usersToDelete = await pool.query('SELECT user_id FROM usuarios_propiedades WHERE propiedad_id = $1', [propiedadId]);
    ...
    app.post('/propiedades-admin/:id/ajustar-saldo', verifyToken, async (req, res, _next) => {
        const propiedadId = asString(req.params.id);
        ...
        try {
            ...
            if (tipo_ajuste === 'AGREGAR_FAVOR' && cuenta_bancaria_id && !es_gasto_extra) {
                const fondosRes = await pool.query('SELECT id, moneda FROM fondos WHERE cuenta_bancaria_id = $1 AND activo = true ORDER BY es_operativo DESC, id ASC', [cuenta_bancaria_id]);
                if (fondosRes.rows.length > 0) {
                    ...
                    await pool.query('INSERT INTO movimientos_fondos (fondo_id, tipo, monto, tasa_cambio, nota) VALUES ($1, $2, $3, $4, $5)', [f.id, 'INGRESO', montoFondo, parseFloat(String(tasa_cambio || '1')) || 1, nota || 'Ajuste manual a favor']);
                }
            }
            else if (tipo_ajuste === 'AGREGAR_FAVOR' && es_gasto_extra && gasto_extra_id) {
                // Abono directo para rebajar la deuda del gasto extra
                await pool.query('UPDATE gastos SET monto_pagado_usd = COALESCE(monto_pagado_usd, 0) + $1 WHERE id = $2', [montoNum, gasto_extra_id]);
            }
            await pool.query('COMMIT');
