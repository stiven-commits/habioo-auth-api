...
const registerPagosRoutes = (app, { pool, verifyToken, parseLocaleNumber, getPagosOptionalColumns }) => {
    const resolveMovimientoFondoTipo = async (preferred, fallback) => {
        try {
    ...
    const round2 = (n) => parseFloat((parseFloat(String(n || 0))).toFixed(2));
    ...
    const userHasPropiedadPortalAccess = async (userId, propiedadId) => {
        const r = await pool.query(`
              SELECT 1 AS ok
    ...
    const adminHasPropiedadAccess = async (adminUserId, propiedadId) => {
        const r = await pool.query(`
              SELECT 1 AS ok
              FROM propiedades p
    ...
    const parseMesCobroToYyyyMm = (mesCobro) => {
        if (!mesCobro)
            return null;
    ...
    const buildReciboBreakdownByGasto = async (client, recibo) => {
        ...
        const zonasRes = await client.query('SELECT zona_id FROM propiedades_zonas WHERE propiedad_id = $1', [propiedadId]);
        const zonaIds = new Set(zonasRes.rows.map((z) => z.zona_id));
        const totalPropsRes = await client.query('SELECT COUNT(*)::int AS total FROM propiedades WHERE condominio_id = $1', [condominioId]);
        ...
        for (const c of cuotasRes.rows) {
            ...
            if (c.tipo === 'Comun' || c.tipo === 'Extra') {
                ...
                if (c.zona_id === null || !zonaIds.has(c.zona_id))
                    continue;
                if (metodoDivision === 'Partes Iguales') {
                    const propsZonaRes = await client.query('SELECT COUNT(*)::int AS total FROM propiedades_zonas WHERE zona_id = $1', [c.zona_id]);
                    const totalZona = parseInt(String(propsZonaRes.rows[0]?.total || 0), 10);
                    share = totalZona > 0 ? (montoCuota / totalZona) : 0;
                }
                else {
                    const sumAlRes = await client.query(`
    ...
    const imputarReciboEnGastos = async (client, recibo, montoAplicadoUsd) => {
        const montoAplicado = round2(montoAplicadoUsd);
        if (montoAplicado <= 0)
            return { montoExtraUsd: 0, montoDistribuibleUsd: 0 };
    ...
    const distribuirAbonoEnFondos = async (client, { cuentaId, montoDistribuibleUsd, montoDistribuibleOrigen, monedaPago, tasaNum, pagoId, referencia }) => {
        ...
        if (montoBase <= 0)
            return;
        const fondosRes = await client.query(`
        ...
        for (const d of distBase) {
            ...
            await client.query('INSERT INTO movimientos_fondos (fondo_id, tipo, monto, referencia_id, tasa_cambio, nota) VALUES ($1, $2, $3, $4, $5, $6)', [d.id, tipoMovimiento, montoFondo, pagoId, tasaNum || null, `Abono distribuible de pago${pagoId ? ` #${pagoId}` : ''} (${referencia || 'sin referencia'})`]);
        }
    };
    const aplicarPagoEnCascada = async (client, pago) => {
        ...
        const recibosPendientes = await client.query("SELECT * FROM recibos WHERE propiedad_id = $1 AND estado != 'Pagado' ORDER BY fecha_emision ASC, id ASC", [propiedadId]);
    ...
    app.post('/pagos-admin', verifyToken, async (req, res, _next) => {
        ...
        try {
            ...
            const notaOrigenPartes = [];
            if (bancoOrigenSafe)
                notaOrigenPartes.push(`Banco origen: ${bancoOrigenSafe}`);
            if (cedulaOrigenSafe)
                notaOrigenPartes.push(`Cedula origen: ${cedulaOrigenSafe}`);
            if (telefonoOrigenSafe)
                notaOrigenPartes.push(`Telefono origen: ${telefonoOrigenSafe}`);
            ...
            const pagoInsertRes = await pool.query(`INSERT INTO pagos (${insertColumns.join(', ')}) VALUES (${placeholders}) RETURNING id`, insertValues);
            ...
            let montoDistribuibleFondos = 0;
            let montoExtraTotalUsd = 0;
            const recibosPendientes = await pool.query("SELECT * FROM recibos WHERE propiedad_id = $1 AND estado != 'Pagado' ORDER BY fecha_emision ASC, id ASC", [propiedad_id]);
            for (const rec of recibosPendientes.rows) {
                if (dineroRestante <= 0)
                    break;
                const deudaRecibo = round2(parseFloat(String(rec.monto_usd || 0)) - parseFloat(String(rec.monto_pagado_usd || 0)));
                ...
                if (dineroRestante >= deudaRecibo) {
                    ...
                    await pool.query("UPDATE recibos SET monto_pagado_usd = COALESCE(monto_pagado_usd, 0) + $1, estado = 'Abonado' WHERE id = $2", [dineroRestante, rec.id]);
                    dineroRestante = 0;
                }
    ...
    app.post('/pagos-propietario', verifyToken, async (req, res, _next) => {
        const { propiedad_id, cuenta_id, monto_origen, tasa_cambio, referencia, fecha_pago, nota, cedula_origen, banco_origen, telefono_origen, moneda, recibo_id, metodo } = req.body;
        try {
            const user = asAuthUser(req.user);
            const propiedadIdNum = parseInt(String(propiedad_id), 10);
            if (!Number.isFinite(propiedadIdNum) || propiedadIdNum <= 0) {
            ...
            const placeholders = insertValues.map((_, i) => `$${i + 1}`).join(', ');
            await pool.query(`INSERT INTO pagos (${insertColumns.join(', ')}) VALUES (${placeholders})`, insertValues);
            await pool.query('COMMIT');
    ...
    app.get('/pagos/pendientes-aprobacion', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
    ...
    app.post('/pagos/:id/validar', verifyToken, async (req, res, _next) => {
        const pagoId = asString(req.params.id);
        try {
            const user = asAuthUser(req.user);
            await pool.query('BEGIN');
            const pagoRes = await pool.query("SELECT id, monto_usd, monto_origen, tasa_cambio, propiedad_id, estado, cuenta_bancaria_id, moneda, referencia FROM pagos WHERE id = $1 AND estado IN ('Pendiente', 'PendienteAprobacion')", [pagoId]);
    ...
    app.post('/pagos/:id/rechazar', verifyToken, async (req, res, _next) => {
        const pagoId = asString(req.params.id);
        ...
        try {
            const user = asAuthUser(req.user);
            const pagoRes = await pool.query("SELECT id, monto_usd, monto_origen, tasa_cambio, propiedad_id, estado, cuenta_bancaria_id, moneda, referencia FROM pagos WHERE id = $1 AND estado = 'PendienteAprobacion'", [pagoId]);
            if (pagoRes.rows.length === 0) {
            ...
            const optionalCols = await getPagosColumns();
            if (optionalCols.nota) {
                await pool.query("UPDATE pagos SET estado = 'Rechazado', nota = CONCAT(COALESCE(NULLIF(TRIM(nota), ''), ''), CASE WHEN COALESCE(NULLIF(TRIM(nota), ''), '') = '' THEN '' ELSE ' | ' END, 'Rechazado: ', $2::text) WHERE id = $1", [pagoId, notaRechazo]);
            }
            else {
    ...
    app.get('/pagos-proveedores/gasto/:gasto_id/detalles', verifyToken, async (req, res) => {
        try {
            const user = asAuthUser(req.user);
            ...
            const gastoRes = await pool.query('SELECT id FROM gastos WHERE id = $1 AND condominio_id = $2 LIMIT 1', [gastoId, condoId]);
            ...
            const bancoColsRes = await pool.query(`
                SELECT column_name
                FROM information_schema.columns
                WHERE table_schema = 'public'
            ...
            const buildBancoNombreExpr = (alias) => {
                const candidates = [];
                if (bancoCols.has('nombre_banco'))
                    candidates.push(`${alias}.nombre_banco`);
                if (bancoCols.has('nombre'))
                    candidates.push(`${alias}.nombre`);
                if (bancoCols.has('banco'))
                    candidates.push(`${alias}.banco`);
                if (bancoCols.has('apodo'))
                    candidates.push(`${alias}.apodo`);
            ...
            `;
            const detalles = await pool.query(query, [gastoId]);
    ...
    app.post('/pagos-proveedores', verifyToken, async (req, res) => {
        ...
        try {
            ...
            const origenesNormalizados = origenes.map((origen) => {
            ...
            const montoTotalPagoUsd = round2(origenesNormalizados.reduce((acc, origen) => acc + origen.montoUsd, 0));
            const montoTotalPagoUsdExact = origenesNormalizados.reduce((acc, origen) => acc + (Number.isFinite(origen.montoUsdExact) ? origen.montoUsdExact : 0), 0);
            if (montoTotalPagoUsd <= 0) {
                return res.status(400).json({ status: 'error', message: 'El pago total en USD debe ser mayor a 0.' });
            }
            await pool.query('BEGIN');
            txStarted = true;
            const gastoUpdate = await pool.query(`
                UPDATE gastos
            ...
            const totalBs = round2(origenesNormalizados.reduce((acc, origen) => acc + origen.montoOrigen, 0));
            ...
            const pagoProveedorRes = await pool.query(`
            ...
            const cuentaSaldoColRes = await pool.query(`
                SELECT column_name
                FROM information_schema.columns
                WHERE table_schema = 'public'
                  AND table_name = 'cuentas_bancarias'
            ...
            const gpfColsRes = await pool.query(`
                SELECT column_name
                FROM information_schema.columns
                WHERE table_schema = 'public'
                  AND table_name = 'gastos_pagos_fondos'
            ...
            for (const origen of origenesNormalizados) {
                if (cuentaSaldoCol) {
                    await pool.query(`UPDATE cuentas_bancarias SET ${cuentaSaldoCol} = COALESCE(${cuentaSaldoCol}, 0) - $1 WHERE id = $2`, [origen.montoOrigen, origen.cuentaId]);
                }
                if (origen.fondoId) {
                    const fondoRes = await pool.query(`
                    ...
                    if (saldoActualFondo + 0.0001 < montoDebitarFondo) {
                        throw new Error(`Saldo insuficiente en el fondo ${origen.fondoId}. Disponible: ${saldoActualFondo}, requerido: ${montoDebitarFondo}.`);
                    }
                ...
                const gpfInsertCols = ['gasto_id', 'fondo_id', 'monto_pagado_usd', 'fecha_pago'];
                const gpfInsertVals = [gasto_id, origen.fondoId, origen.montoUsd, fechaPagoSafe];
                ...
                await pool.query(`INSERT INTO gastos_pagos_fondos (${gpfInsertCols.join(', ')}) VALUES (${gpfPlaceholders})`, gpfInsertVals);
            }
            await pool.query('COMMIT');
            ...
            if (txStarted)
                await pool.query('ROLLBACK');
            const isBusinessError = error.message.includes('monto_') ||
