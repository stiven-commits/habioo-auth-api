...
// ─── Registro de rutas ────────────────────────────────────────────────────────
const registerEncuestasRoutes = (app, { pool, verifyToken }) => {
    /**
     * POST /encuestas
     ...
     */
    app.post('/encuestas', verifyToken, async (req, res) => {
        ...
        try {
            ...
            const cRes = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [authUser.id]);
            ...
            const eRes = await pool.query(`INSERT INTO encuestas (condominio_id, titulo, descripcion, tipo, fecha_fin)
                     VALUES ($1, $2, $3, $4, $5) RETURNING id`, [condominio_id, titulo, descripcion ?? null, tipo, fecha_fin]);
            const encuesta_id = eRes.rows[0].id;
            if (tipo === 'SI_NO') {
                await pool.query('INSERT INTO encuesta_opciones (encuesta_id, texto) VALUES ($1, $2)', [encuesta_id, 'Sí']);
                await pool.query('INSERT INTO encuesta_opciones (encuesta_id, texto) VALUES ($1, $2)', [encuesta_id, 'No']);
            }
            else if (tipo === 'MULTIPLE' && Array.isArray(opciones) && opciones.length > 0) {
                for (const texto of opciones) {
                    await pool.query('INSERT INTO encuesta_opciones (encuesta_id, texto) VALUES ($1, $2)', [encuesta_id, texto]);
                }
            }
     ...
     */
    app.get('/encuestas/:condominio_id', verifyToken, async (req, res) => {
        const { condominio_id } = req.params;
        const { propiedad_id } = req.query;
        try {
            asAuthUser(req.user);
            const eRes = await pool.query(`SELECT id, condominio_id, titulo, descripcion, tipo, fecha_fin, created_at
                     FROM encuestas
                     WHERE condominio_id = $1
                     ORDER BY created_at DESC`, [condominio_id]);
            const encuestas = [];
            for (const encuesta of eRes.rows) {
                const opRes = await pool.query(`SELECT id, encuesta_id, texto
                         FROM encuesta_opciones
                         WHERE encuesta_id = $1
                         ORDER BY id ASC`, [encuesta.id]);
                let ya_voto = false;
                if (propiedad_id) {
                    const vRes = await pool.query(`SELECT COUNT(*) AS count
                             FROM encuesta_votos
                             WHERE encuesta_id = $1 AND propiedad_id = $2`, [encuesta.id, propiedad_id]);
                    ya_voto = parseInt(vRes.rows[0].count, 10) > 0;
                }
                const tvRes = await pool.query('SELECT COUNT(*) AS count FROM encuesta_votos WHERE encuesta_id = $1', [encuesta.id]);
                encuestas.push({
                    ...encuesta,
                    opciones: opRes.rows,
                    ya_voto,
     ...
     */
    app.post('/encuestas/:id/votar', verifyToken, async (req, res) => {
        const { id } = req.params;
        ...
        try {
            ...
            const eRes = await pool.query('SELECT id, tipo, fecha_fin FROM encuestas WHERE id = $1', [id]);
            ...
            const dupRes = await pool.query(`SELECT COUNT(*) AS count
                     FROM encuesta_votos
                     WHERE encuesta_id = $1 AND propiedad_id = $2`, [id, propiedad_id]);
            ...
            await pool.query(`INSERT INTO encuesta_votos (encuesta_id, propiedad_id, user_id, opcion_id, respuesta_texto)
                     VALUES ($1, $2, $3, $4, $5)`, [id, propiedad_id, authUser.id, opcion_id ?? null, respuesta_texto ?? null]);
     ...
     */
    app.get('/encuestas/:id/resultados', verifyToken, async (req, res) => {
        const { id } = req.params;
        try {
            const authUser = asAuthUser(req.user);
            const eRes = await pool.query('SELECT id, titulo, tipo FROM encuestas WHERE id = $1', [id]);
            if (eRes.rows.length === 0) {
                res.status(404).json({ status: 'error', message: 'Encuesta no encontrada.' });
                return;
            }
            ...
            const condoRes = await pool.query(`SELECT metodo_division
                     FROM condominios
                     WHERE id = (SELECT condominio_id FROM encuestas WHERE id = $1)`, [id]);
            ...
            const conteoRes = await (metodo_division === 'Alicuota'
                ? pool.query(`SELECT ev.opcion_id,
            ...
            if (authUser.is_admin) {
                ...
                const detalleRes = await pool.query(`SELECT ev.opcion_id,
                                eo.texto          AS opcion_texto,
                                ev.respuesta_texto,
                ...
                const textosRes = await pool.query(`SELECT respuesta_texto
                         FROM encuesta_votos
