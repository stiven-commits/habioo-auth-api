...
const asDecimal = (value, fieldName) => {
...
const asOptionalStringOrNull = (value) => {
...
const registerBancosRoutes = (app, { pool, verifyToken }) => {
    app.get('/bancos', verifyToken, async (req, res, _next) => {
        try {
            ...
            'SELECT cb.* FROM cuentas_bancarias cb JOIN condominios c ON cb.condominio_id = c.id WHERE c.admin_user_id = $1 AND cb.activo = true ORDER BY cb.nombre_banco ASC', [user.id]);
    ...
    app.post('/bancos', verifyToken, async (req, res, _next) => {
        const { numero_cuenta, nombre_banco, apodo, tipo, nombre_titular, cedula_rif, telefono, acepta_transferencia, acepta_pago_movil, pago_movil_telefono, pago_movil_cedula_rif, } = req.body;
        try {
            const user = asAuthUser(req.user);
            ...
            const cuentaCols = await pool.query(`SELECT column_name
                 FROM information_schema.columns
                 WHERE table_schema = 'public' AND table_name = 'cuentas_bancarias'`);
            const hasCuentaCol = (name) => cuentaCols.rows.some((r) => r.column_name === name);
            ...
            const insertCols = ['condominio_id', 'numero_cuenta', 'nombre_banco', 'apodo', 'tipo', 'nombre_titular', 'cedula_rif', 'telefono'];
            const insertVals = [condoId, numeroCuentaSafe || '', nombreBancoSafe || '', apodoSafe, tipoSafe, nombreTitularSafe || '', cedulaRifSafe || '', telefonoSafe || ''];
            ...
            await pool.query(`INSERT INTO cuentas_bancarias (${insertCols.join(', ')}) VALUES (${placeholders})`, insertVals);
    ...
    app.put('/bancos/:id', verifyToken, async (req, res, _next) => {
        const { numero_cuenta, nombre_banco, apodo, tipo, nombre_titular, cedula_rif, telefono, acepta_transferencia, acepta_pago_movil, pago_movil_telefono, pago_movil_cedula_rif, } = req.body;
        try {
            const user = asAuthUser(req.user);
            ...
            const result = await pool.query(`UPDATE cuentas_bancarias
                 SET ${updateSets.join(', ')}
                 WHERE id = $${updateVals.length - 1} AND condominio_id = $${updateVals.length}
                 RETURNING id`, updateVals);
    ...
    app.put('/bancos/:id/predeterminada', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
    ...
    app.delete('/bancos/:id', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const cuentaId = asString(req.params.id);
            const c = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
            ...
            const movs = await pool.query(`SELECT COUNT(*) FROM fondos WHERE cuenta_bancaria_id = $1 AND activo = true`, [cuentaId]);
    ...
    app.get('/bancos-admin/:id/estado-cuenta', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            ...
            const cuentaRes = await pool.query('SELECT id FROM cuentas_bancarias WHERE id = $1 AND condominio_id = $2 LIMIT 1', [cuentaId, condoId]);
            ...
            `);
            const hasCol = (table, column) => colsRes.rows.some((r) => r.table_name === table && r.column_name === column);
            ...
            `;
            const movimientos = await pool.query(query, [cuentaId]);
    ...
    app.post('/transferencias', verifyToken, async (req, res, _next) => {
        ...
        try {
            ...
            await pool.query('BEGIN');
            const origenRes = await pool.query('SELECT id, cuenta_bancaria_id, condominio_id, moneda, porcentaje_asignacion, es_operativo FROM fondos WHERE id = $1 AND condominio_id = $2 AND activo = true LIMIT 1', [fondoOrigenIdSafe, condoId]);
            ...
            const sourceBalanceRes = await pool.query('SELECT saldo_actual FROM fondos WHERE id = $1 AND condominio_id = $2 LIMIT 1', [fondoOrigenIdSafe, condoId]);
            ...
            if (hasCuentaDestino) {
                ...
                const destinoFundsRes = await pool.query(`SELECT id, cuenta_bancaria_id, condominio_id, moneda, porcentaje_asignacion, es_operativo
                     FROM fondos
                     WHERE cuenta_bancaria_id = $1
                       AND condominio_id = $2
                       AND activo = true
                       AND moneda = $3
                     ORDER BY es_operativo DESC, id ASC`, [cuentaDestinoIdSafe, condoId, fondoOrigen.moneda]);
                ...
                for (const dist of distribuciones) {
                    ...
                    await pool.query(`INSERT INTO transferencias (condominio_id, fondo_origen_id, fondo_destino_id, monto_origen, tasa_cambio, monto_destino, referencia, fecha, nota)
                ...
                await pool.query(`INSERT INTO transferencias (condominio_id, fondo_origen_id, fondo_destino_id, monto_origen, tasa_cambio, monto_destino, referencia, fecha, nota)
                     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [condoId, fondoOrigenIdSafe, fondoDestinoIdSafe, montoOrigenSafe, tasaCambioSafe, montoDestinoSafe, referenciaSafe, fechaSafe, notaSafe]);
            }
            await pool.query('COMMIT');
    ...
    app.get('/gastos-pendientes-pago', verifyToken, async (req, res, _next) => {
        try {
            const user = asAuthUser(req.user);
            const condoRes = await pool.query('SELECT id FROM condominios WHERE admin_user_id = $1 LIMIT 1', [user.id]);
            const condoId = condoRes.rows[0].id;
